...
# ═══════════════════════════════════════════════════════════════════════════
# PARTIAL EVIDENCE HELPERS  [PC-1, PC-2, PC-6, PC-7, PC-8]
# ═══════════════════════════════════════════════════════════════════════════

def extract_leading_digit(text: str) -> Optional[str]:
    """
    [PC-8] Extract the optional leading zone digit from a plate string
    or partial OCR string.

    Examples:
      "5กข2662"  → "5"
      "กข2662"   → None
      "5012662"  → "5"   (raw digit string with leading zone digit)
      "ฐบ3699"   → None  (no leading digit)
      "1ฐบ3699"  → "1"
    """
    if not text:
        return None
    ...
    # Already a valid plate with leading digit
    m = _PLATE_RE.fullmatch(norm)
    if m and m.group(1):
        return m.group(1)

    # Raw digit string: first char is zone digit if string is 7+ chars
    # Pattern: D CC DDDD  (7 chars) or D D D D D D D (7 digits where 2nd/3rd = consonants)
    if len(norm) >= 7 and norm[0].isdigit():
        # Check if rest looks like a plate (has consonants or correctable)
        rest = norm[1:]
        if any(c in _CONSONANT_SET for c in rest):
            return norm[0]
        # All digits: first is zone, positions 1-2 are consonant-encoded
...
def extract_digit_suffix(text: str) -> Optional[str]:
    ...
    Examples:
      ...
      "5กข2662"    → "2662"  (valid plate with leading digit)
...
def merge_digit_evidence(
    ...
    leading_digit: Optional[str] = None,
) -> list:
    """
    [PC-2, PC-7] Combine a confirmed digit suffix with consonant candidates
    observed across frames in the same track to produce full plate strings.
    Now accepts an optional leading_digit to reconstruct zone-prefixed plates
    like 5กข2662 instead of กข2662.
    ...
    leading_digit       : optional single digit zone prefix (e.g. "5")
    ...
    for cons in consonant_candidates:
        ...
        if len(cons) == 2 and all(c in _CONSONANT_SET for c in cons):
            # Try with leading digit first (preferred if available)
            if leading_digit and leading_digit.isdigit():
                candidate = leading_digit + cons + digit_suffix
                if _PLATE_RE.fullmatch(candidate) and candidate not in seen:
                    results.append(candidate)
                    seen.add(candidate)
            # Also try without leading digit
            candidate = cons + digit_suffix
            if _PLATE_RE.fullmatch(candidate) and candidate not in seen:
                results.append(candidate)
                seen.add(candidate)

        # 3-char case: leading digit + 2 consonants (e.g. "5กข" or "1กข")
        elif (len(cons) == 3
              and cons[0].isdigit()
            ...
            if _PLATE_RE.fullmatch(candidate) and candidate not in seen:
                results.append(candidate)
                seen.add(candidate)
            # Also try overriding the embedded leading digit with the
            # separately-tracked one if it differs
            if leading_digit and leading_digit.isdigit() and leading_digit != cons[0]:
                candidate2 = leading_digit + cons[1:] + digit_suffix
                if _PLATE_RE.fullmatch(candidate2) and candidate2 not in seen:
